/**
*、与えられた入力文字列を解析して、それを指定された形式に変換する
  *
  * - `Parser`クラスのコンストラクタは、`input`と`root`の2つの引数を受け取ります。`input`は解析する入力文字列であり、`root`は解析結果のルートとなるオブジェクトです。
  *
  * - `mode_list`は、解析中の状態（モード）を定義します。各状態には、受け入れ可能なコマンド、受け入れ可能なデリミタ、エラーメッセージ、完了フラグなどが含まれます。
  *
  * - `accept_handlers`は、受け入れ可能なデリミタごとに対応するハンドラと次の状態を定義します。
  *
  * - `parse`メソッドは、入力文字列を解析して指定された形式に変換します。具体的には、各コマンドやデリミタをチェックし、それに応じて処理を行います。
  *
  * - `check_command`メソッドは、入力文字列の先頭にあるコマンドをチェックします。与えられた文字列がコマンドの開始部分であるかどうかを確認します。
  *
  * - `add_command_item`メソッドは、見つかったコマンドを解析結果に追加します。
  *
  * - `check_delimiter`メソッドは、入力文字列の先頭にあるデリミタをチェックします。与えられた文字列が受け入れ可能なデリミタかどうかを確認します。
  *
  * - `handle_delimiter`メソッドは、見つかったデリミタに対応するハンドラを呼び出し、次の状態に移行します。
  *
  * - `is_last_container`メソッドは、スタックの一番上の要素が指定されたクラスのインスタンスかどうかを確認します。
  *
  * - `parse_times`メソッドは、繰り返し回数を解析します。
  *
  * - `create_exception_string`メソッドは、例外メッセージを作成し、エラーの発生箇所を示すHTML文字列を返します。
  *
* @param input
* @param root
* @constructor
